...
function layout(w, h, overlay) {
  ...
  return {
    ...
    blocked: [],
    callout: { width: 128, height: 44, dx: 24, kinds: ["start", "ride", "end"] },
  };
}

const shortName = (segment) => segment.name.split(" · ")[0];

function Callout({ item, current, minutesToNext, from, to, arrival, layers, narrow }) {
  ...
  return (
    <div
      ...
      style={{ left: rect.x0, top: rect.y0, width: rect.x1 - rect.x0, minHeight: rect.y1 - rect.y0 }}
      hidden={
        ...
        {map && (
          ...
          >
            <defs>
              ...
              <pattern id="tk-dots" width="9" height="9" patternUnits="userSpaceOnUse">
                <circle cx="1.5" cy="1.5" r="0.8" fill="#4cc9e6" opacity="0.16" />
              </pattern>
              <filter id="tk-glow" filterUnits="userSpaceOnUse" x="0" y="0" width={w} height={h}>
            ...
            <path className="rm-coast-glow" d={map.land} filter="url(#tk-glow)" />
            ...
            {w >= 560 && (
              <text className="tk-island-name" x={w * 0.85} y={h * 0.6} textAnchor="middle">
                SRI LANKA
              </text>
            )}
            ...
            {map.parts.map((part, i) => (
              <g key={`${part.mode}-${i}`} className={`tk-leg ${part.mode}`}>
                <path className="glow" d={toPath(part.points)} filter="url(#tk-glow)" />
            ...
            {layers.stations &&
              map.callouts
                ...
                .map((c) => (
                  <g key={`transfer-${c.segment.start}`} className="tk-transfer">
            ...
            {layers.hubs && (
              <>
                <g className="tk-start">
                  <circle className="halo" cx={map.start[0]} cy={map.start[1]} r="15" />
                  <circle className="dot" cx={map.start[0]} cy={map.start[1]} r="7" />
                </g>
                <MapPin
            ...
            {layers.vehicles &&
              map.nodes.map((node) => {
                ...
                return (
                  <g key={`node-${node.start}`} className={`tk-node ${node.mode} ${isCurrent ? "current" : ""}`}>
                    {isCurrent && <circle className="halo" cx={node.mid[0]} cy={node.mid[1]} r="31" />}
                    <circle className="disc" cx={node.mid[0]} cy={node.mid[1]} r="20" />
                    <ModeIcon mode={node.mode} size={20} x={node.mid[0] - 10} y={node.mid[1] - 10} />
                  </g>
                );
              })}
            ...
            {layers.vehicles && map.vehicle && (
              <g className={`tk-vehicle ${currentMode || ""}`}>
                <circle className="pulse" cx={map.vehicle[0]} cy={map.vehicle[1]} r="9" />
                <circle className="core" cx={map.vehicle[0]} cy={map.vehicle[1]} r="6" />
              </g>
            )}
          </svg>
        )}
            ...
            {LAYERS.map(({ id, label, icon: Icon }) => (
              <li key={id}>
                ...
                <Switch
                  ...
                  onChange={(on) => setLayers((current) => ({ ...current, [id]: on }))}
                />
              </li>
            ))}
          </ul>
